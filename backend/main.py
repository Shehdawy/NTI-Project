...
from backend.services.pricing_service import explain, simulate
...
DEFAULT_ALLOWED_ORIGINS = "http://localhost:8501,http://localhost:8502,http://localhost:8503"


@asynccontextmanager
async def lifespan(_: FastAPI):
    """Load the trained model once at startup, if it's present on disk."""
...
app = FastAPI(
    title="AI Retail Pricing API",
    version="1.0.0",
    description="Demand prediction and price simulation using the existing trained retail model.",
    lifespan=lifespan,
)
app.add_middleware(
    CORSMiddleware,
...
async def unexpected_error(_: Request, __: Exception):
    """Catch-all so unexpected errors never leak internal details to clients."""
    return JSONResponse(
        status_code=500,
        content={"error": {"code": "PREDICTION_ERROR", "message": "The prediction service could not complete the request."}},
    )
...
def root():
    return {
        "name": "AI Retail Pricing API",
        "version": "1.0.0",
        "status": "running",
        "description": "AI-powered retail demand and price recommendation API",
    }
...
def model_info():
    if not MODEL_SERVICE.loaded:
        return JSONResponse(
            status_code=503,
            content={"error": {"code": "MODEL_UNAVAILABLE", "message": "The demand model is not loaded."}},
        )
    return ModelInfoResponse(
        model_type=MODEL_SERVICE.metadata.get("best_model", "Saved demand model"),
...
def recommend(request: RecommendationRequest):
    ...
    scenarios = [
        Scenario(
            ...
            expected_profit=float(row.expected_profit) if pd.notna(row.expected_profit) else None,
        )
    ...
    return RecommendationResponse(
        ...
        explanation=explain(request, result),
    )
